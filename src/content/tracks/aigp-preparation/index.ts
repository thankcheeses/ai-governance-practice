...
/**
 * Convert letter-keyed distractor type labels to option ids, mirroring
 * normalizeDistractorNotes. Returns undefined when no types are present.
 */
function normalizeDistractorTypes(
  questionId: string,
  types: Partial<Record<OptionKey, DistractorType>> | undefined,
): Partial<Record<string, DistractorType>> | undefined {
  if (!types) return undefined;
  const out: Partial<Record<string, DistractorType>> = {};
  for (const [key, value] of Object.entries(types)) {
    const index = OPTION_KEYS.indexOf(key as OptionKey);
    if (index >= 0 && value) out[optionId(questionId, index)] = value;
  }
  return Object.keys(out).length ? out : undefined;
}

/**
 * Normalize optional reasoning metadata: convert letter-keyed distractor
 * types to option ids. Returns undefined when enrichment has no reasoning.
 */
function normalizeReasoning(
  ...
  const distractorTypes = normalizeDistractorTypes(
    questionId,
    meta.distractorTypes,
  );
  return {
    primaryDimension: meta.primaryDimension,
    ...(meta.secondaryDimensions?.length
      ? { secondaryDimensions: meta.secondaryDimensions }
      : {}),
    ...(distractorTypes ? { distractorTypes } : {}),
  };
}
...
function normalize(item: RawQuestion): Question {
  ...
  const reasoning = normalizeReasoning(id, enrichment.reasoning);
